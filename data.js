    ...
    reader.onload = (e) => {
      try {
        ...
        try {
          ...
          if (fun) {
            const parseFunilBlock = (raw, key1, key2) => {
              let startIdx = -1;
              for(let i=0; i<raw.length; i++) {
                const cell = String(raw[i]?.[0]||'').toLowerCase();
                if (key2) {
                  if (cell.includes(key1) && cell.includes(key2)) { startIdx = i + 1; break; }
                } else {
                  if (cell.includes(key1)) { startIdx = i + 1; break; }
                }
              }
              ...
              for (let i = startIdx; i < raw.length; i++) {
                ...
                if (cell === '') continue;
                // Blinda a leitura: se achar um header de outro bloco ou "Total", para a leitura deste bloco
                if (cell.toLowerCase().includes('dias') || cell.toLowerCase().includes('geral') || cell.toLowerCase().includes('total') || cell.toLowerCase().includes('funil')) break;
                
                result.push({
                  op: cell,
                  disc: parseNum(row[1]), alo: parseNum(row[2]), alo_pct: parseNum(row[3]),
                  cpc: parseNum(row[4]), cpc_pct: parseNum(row[5]),
                  prom: parseNum(row[6]), prom_pct: parseNum(row[7])
                });
              }
              return result;
            };

            const ag = parseFunilBlock(fun, 'geral');
            if (ag.length) DADOS.funil.abrilGeral = ag;
            const m9 = parseFunilBlock(fun, 'maio', '9');
            if (m9.length) DADOS.funil.maio9Dias = m9;
            const a9 = parseFunilBlock(fun, 'abril', '9');
            if (a9.length) DADOS.funil.abril9Dias = a9;
          }
